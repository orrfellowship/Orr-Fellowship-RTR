...
check("keeps legacy tokens valid for saved templates", findUnsupportedOutreachVariables("Hi {{first_name}} — {{point_person}}").length === 0);
...
check("normalizes legacy tokens to their new names", normalizeOutreachMergeVariables("Hi {{ first_name }} {{last_name}} — {{POINT_PERSON}}") === "Hi {{candidate_first_name}} {{candidate_last_name}} — {{fellow_point_person}}");
...
check("tokens derive candidate first/last/full", tokens.candidate_first_name === "Catherine" && tokens.candidate_last_name === "Mazanek" && tokens.full_name === "Catherine Mazanek");
...
check("still renders legacy aliases", renderOutreachTemplate("Hi {{first_name}} — {{point_person}}", tokens) === "Hi Catherine — Mark Stolte");
